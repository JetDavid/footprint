...
> 以下内容均来自*Large-scale cluster management at Google with Borg*，通过阅读文章，总结和梳理了在面对大规模集群设计时所要考虑的方方面面。对充分理解kubernetes设计背景和当前主要开发和维护的IaaS资源管理集群有很多借鉴意义。但在学习中也要注意Borg并不是面向IaaS平面的资源管理系统，需注意区分场景和需求的不同
...
1. Borg使用了e-pvm实现分配，即每个节点均衡分配。但会导致大资源消耗的task无法分配。这是最差匹配
2. 最佳匹配则是尽量充分利用一个整机资源，分配完成后，再分配下一个。但这个会导致某些业务的突增性能无法达到。
3. 当前为混合分配模式，混合模式为了解决搁浅资源（该节点已经被分配了所有资源，但是实际上task并未充分利用）。混合分配方式比最佳匹配提高了3-5%整合效率。
...
但对于IaaS计算不提供竞价性实例的情况下，“抢占”导致的退出并不存在。因此大头在机器故障和意外异常宕机。增强可靠性措施，常见IaaS管理平台与borg对比：
...
整体对比来看，可靠性保障上，常规IaaS平台与早期的borg差别不大。
...
如下图所示，图中可见当cell资源使用情况保持在10%以下，20%左右，60%左右都呈现出较为弹性的可压缩空间。这个结果对于其他分布式资源管理系统是不太有借鉴意义的。个人认为，cell压缩可以参考，但因业务场景而变。对于IaaS基础设施资源管理调度系统，大部分成本来自于硬件（服务器中cpu, memory，GPU等）和数据中心设施（机架，场地，管理费用等）这部分成本一旦付出，不具有较为灵活的弹性。当然对于PaaS层面的业务，kubernetes等集群，利用IaaS提供的弹性和低成本实例（竞价性，突发型，共享型实例）具有很好成本优化的借鉴意义。

![effects of compaction](images/effects-of-compaction.png)

### 3.2 Cell共享

下图是混合部署生产与非生产业务所呈现的cell规模对比。 混合要比不混合节省大概20%-30的机器成本。这一点对于kubernetes和常规IaaS平台而言也是有不同的：

- kubernetes使用resource request和limit可以区别业务资源使用硬上限和软上限，从而依赖它们完成所谓“生产和非生产”的任务类型的调度部署。
- IaaS平台为虚拟机调度时，需要根据虚拟机的工作负载来进行细化分析（对外承诺售卖的资源个数，例如2个vCPU = 2个超线程， 8GB mem = 8GB RAM，不可超卖，或不能让客户感知超卖），区分Guaranteed和BestEffort实例。目前来看，可以利用突发型，共享型，竞价性实例来填补常规实例的资源空缺。也就是Borg文档前面提到的“搁浅资源”。但这也大大增加了调度系统的设计难度
...
2. Borg在调查中，并没有明确表达每个task所需的cpu mem资源数目的。按照经验来说，一个服务器一般提供32c到96c不等的cpu。那么如果每个task的cpu需求大，且数量不一，一定会造成该服务器资源碎片的产生，且对于大型计算任务，小型集群不能完全满足，需要增加机器。因此集群越大一定有利于资源调度的有效性和资源利用率提升（资源需求小的task可以消耗碎片）。
3. 对于IaaS和kuberntes来说，具体的借鉴意义是什么？
    1. 大集群资源利用率高
...
上图对比了任务按照细粒度分配资源（将1个cpu切分成毫核）比按照规格分配cpu mem在资源利用率上更加优越。按规格分配面向相同task请求时会消耗更多的机器。这点对于IaaS提供虚拟机和容器的计算规格来看，这一点是无法优化的。
...
- 调度器对生产job按照其申请的资源上限（limit）作为调度依据。不会列入超卖范畴
- 调度器对非生产job按照按照计算所得预留值进行调度和分配。因此一部分资源可以回收
...
Borg同样做了有关保证隔离性的设计，避免tasks之间在运行时产生的相互干扰，主要两个方面：性能和安全。对比IaaS平台有如下表格：

|       | Borg                    | IaaS平台       | Kubernetes
|-------|-------------------------|-----------|------------
...
个人理解对于目前较为成熟的大规模集群来讲，Borg带来以下几个方面值得借鉴和思考：

1. 调度器的优化，包括缓存评分，宽松化随机筛选，等效类别等方法
2. 资源利用率提升，混合分配方式，基于数据分析的动态预留值。为业务定义类别，帮助调度系统有区别，有针对性的调度和分发等等。此外还有CPI和资源利用率上的权衡考量
3. 隔离性，性能上做到更细粒度的判别，例如：numa, L3 cache污染，可压缩资源和不可压缩资源等等
4. 集群理论依据的数据来源，观测手段，统计方法。
